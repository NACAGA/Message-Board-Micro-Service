...
async function convertQueryResultToPosts(queryResult) {
  const posts = [];
...
async function createPost(req) {
  try {
    const validateGroupExistsResult = await validateGroupExists(
      req.params.groupid
    );
    ...
    const verifyUserIsInGroupResult = await verifyUserIsInGroup(
      req.params.userid,
      req.params.groupid
    );
    ...
    const createPostResult = await createPostQuery(
      req.params.userid,
      req.params.groupid,
      req.body.content,
      postedOnDate
...
async function createPostQuery(userid, groupid, content, postedOnDate) {
  const queryResult = await db.query(
    "INSERT INTO Posts (user_id, group_id, content, created_at) VALUES (?, ?, ?, ?)",
    [userid, groupid, content, postedOnDate]
  );
...
async function getPostById(postid) {
  ...
  const queryResult = await db.query("SELECT * FROM Posts WHERE id = ?", [
    idToLookFor,
  ]);
  if (queryResult.result.length === 0) {
  ...
  const post = await convertQueryResultToPosts(queryResult);
  return new GetPostsSuccess(post);
}
...
async function getPostsByUserId(userid) {
  ...
  const userExists = await db.query("SELECT * FROM GroupMembers WHERE id = ?", [
    idToLookFor,
  ]);
  ...
  const queryResult = await db.query("SELECT * FROM Posts WHERE user_id = ?", [
    idToLookFor,
  ]);
  const posts = await convertQueryResultToPosts(queryResult);
...
async function getPostsByGroupId(groupid) {
  ...
  const groupExists = await db.query("SELECT * FROM Groups WHERE id = ?", [
    idToLookFor,
  ]);
  ...
  const queryResult = await db.query("SELECT * FROM Posts WHERE group_id = ?", [
    idToLookFor,
  ]);
  const posts = await convertQueryResultToPosts(queryResult);
  return new GetPostsSuccess(posts);
...
async function getPostsByDateQuery(days) {
  const queryResult = await db.query(
    "SELECT * FROM Posts WHERE created_at >= DATE_SUB(NOW(), INTERVAL ? DAY)",
    [days]
  );
  return queryResult;
}
